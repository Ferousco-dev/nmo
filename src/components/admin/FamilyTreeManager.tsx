...
export function FamilyTreeManager({ initialMembers }: { initialMembers: FamilyTreeRow[] }) {
  ...
  const submit = async () => {
    if (!picked) {
      setErrorMsg('請先搜尋並選擇一位 Skool 成員');
      return;
    }
    if (!role.trim()) {
      setErrorMsg('請輸入職位');
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.ok) {
        setErrorMsg(data.error || '新增失敗');
        return;
      }
      setMembers((prev) => [...prev, data.member as FamilyTreeRow]);
      setSuccessMsg(`已加入 ${data.member?.name ?? picked.displayName ?? picked.handle}`);
      reset();
    } catch {
  ...
  const remove = async (id: string) => {
    if (!confirm('確定要從家族樹移除這位成員嗎？')) return;
    const prev = members;
  ...
  const noResults = !searching && results !== null && results.length === 0 && query.trim().length >= MIN_QUERY_LEN;

  return (
    <div className="space-y-8">
      {/* ====== Add form ====== */}
      <section className="card-premium p-6">
        <h2 className="font-display text-xl font-bold mb-1">新增家族樹成員</h2>
        <p className="text-sm text-ink-muted mb-5">
          先搜尋 Skool 用戶名稱，選擇後輸入職位即可。頭像與名稱會自動從 Skool 同步。
        </p>

        {/* Picked card OR search */}
        {picked ? (
          <div className="flex items-center gap-3 p-3 rounded-lg border border-accent/40 bg-accent/5 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-ink truncate">{picked.displayName ?? picked.handle}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
          <div>
            ...
            <input
              ...
              placeholder="Admin / COO / Regional Commander…"
          ...
          <div>
            ...
            <input
              ...
              placeholder="Taiwan / Hong Kong / Global…"
              className="w-full px-3 h-10 rounded-lg bg-bg-raised/60 border border-line focus:border-accent focus:ring-1 focus:ring-accent/30 outline-none text-sm"
            />
          </div>
        </div>
        ...
        {errorMsg && (
          <p className="mb-3 text-sm text-red-400 font-medium">{errorMsg}</p>
        )}
        {successMsg && (
      ...
      <section>
        <div className="flex items-center gap-3 mb-4">
          <h2 className="font-display text-xl font-bold">目前家族樹成員</h2>
        ...
        {members.length === 0 ? (
          <p className="text-sm text-ink-muted">尚未加入任何成員。</p>
        ) : (
