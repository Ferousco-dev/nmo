  ...
  if (!profile?.skool_handle) {
    return (
          ...
          <div className="card-premium p-8 text-center">
            <Activity className="h-12 w-12 text-accent mx-auto mb-4" aria-hidden />
            <h1 className="font-display text-2xl font-bold mb-2">{t.activity.title}</h1>
            <p className="text-ink-muted mb-6">{t.activity.linkSkoolFirstDesc}</p>
            <Link href="/onboarding" className="text-accent hover:text-accent-glow font-medium">
              {t.activity.linkSkoolFirstCta}
            </Link>
          </div>
        </main>
    );
  ...
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="card-premium p-6">
            ...
            <div className="text-xs text-ink-dim mt-1">/ {allRanks.length} {t.activity.totalMembersUnit}</div>
          </div>
        </div>
        ...
        <div className="card-premium p-5 mb-6 flex items-center justify-between gap-4">
          ...
          {lastBotRun && (
            <div className="text-right text-xs text-ink-dim">
              <div>{(t.activity.botPostsSeen as string).replace('{n}', String(lastBotRun.posts_seen ?? 0))}</div>
              <div>{(t.activity.botEventsInserted as string).replace('{n}', String(lastBotRun.events_inserted ?? 0))}</div>
            </div>
          )}
        </div>
                  ...
                  <li key={e.id} className="flex items-start gap-3 p-3 rounded-lg bg-bg-raised border border-line-subtle">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-ink">
                        ...
                        <span className="ml-2 text-xs text-ink-dim">+{weight} {t.activity.pointsUnit}</span>
                      </div>
                      {e.content_excerpt && (
