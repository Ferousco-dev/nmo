    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="card-premium p-6 md:p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-ink-muted mt-1">第 {currentDay} 天</p>
            </div>
            <Link
              href="/roadmap"
              className="inline-flex items-center gap-1.5 text-sm text-accent hover:text-accent-glow font-medium"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
          ...
          <a
            href={profile.skool_url || 'https://www.skool.com/nmo'}
            target="_blank"
  ...
  return (
    <div className="card-premium p-5">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-ink-muted uppercase tracking-wider font-mono">{label}</span>
        {icon}
      </div>
      <div className="font-display text-3xl font-bold text-ink">{value}</div>
      {sublabel && <div className="text-xs text-ink-dim mt-1">{sublabel}</div>}
    </div>
  );
}
