...
// Server-side admin guard: middleware already blocks non-admins from
// reaching /admin pages, but API routes need their own check.
...
export async function POST(req: Request) {
  ...
  if (authErr) return NextResponse.json({ ok: false, error: authErr }, { status: authErr === 'forbidden' ? 403 : 401 });
  ...
  // handle as a placeholder name — the next Apify sync will fill it.
